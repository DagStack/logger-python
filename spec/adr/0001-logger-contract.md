...
Loggers **MUST** automatically inject into LogRecord:
...
**Opt-out** (rare, for fire-and-forget emits not bound to caller context):

```
logger.withoutContext().info(...)
```

Not inherited further (children default to re-enabled).
...
logging:
  ...
  # Processor chain (Phase 2)
  processors:
    - type: redaction
      extra_patterns: []        # base patterns from config-spec/_meta/secret_patterns.yaml
    - type: sampler_rate
...
- **Extension patterns** — per-deployment in `logging.processors[].type=redaction.extra_patterns`.
...
### 14.6. Phase 1 minimum vs full conformance
